...
const Breadcrumb = ({
  ...
  const renderLabel = (item, isLast) => {
    ...
    return (
      <LinkComponent
        {...linkProps}
        className="text-[var(--muted)] hover:text-[var(--primary)] transition-colors rounded -mx-1 px-1 py-0.5 hover:bg-[var(--surface-hover)]"
      >
        {inner}
      </LinkComponent>
    );
        ...
        {display.map((item, index) => {
          ...
          return (
            <li key={index} className="flex items-center gap-2">
              {index > 0 && <span className="text-[var(--muted)]/60">{sep}</span>}
              {collapsed && index === 1 && (
                <>
                  <span className="text-[var(--muted)]">…</span>
                  <span className="text-[var(--muted)]/60">{sep}</span>
                </>
              )}
